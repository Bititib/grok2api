...
async def video_query(video_id: str) -> dict[str, Any]:
    ...
    async with aiohttp.ClientSession(
        ...
        if (model and (model.startswith("sd2-") or model.startswith("seedance") or model.startswith("sdas-") or model.startswith("ld-"))) or "sd2" in channel_id or "seedance" in channel_id or "sdas" in channel_id or "ld-" in channel_id:
