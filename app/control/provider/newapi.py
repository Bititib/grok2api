...
def is_third_party_video_model(model: str) -> bool:
    ...
    return (
        ...
        or model.startswith("sd2.0-")
        or model.startswith("video-")
    )
...
async def video_query(video_id: str) -> dict[str, Any]:
    ...
    async with aiohttp.ClientSession(
        ...
        if (model and (model.startswith("sd2-") or model.startswith("seedance") or model.startswith("sdas-") or model.startswith("ld-") or model.startswith("sd2.0-") or model.startswith("video-"))) or "sd2" in channel_id or "seedance" in channel_id or "sdas" in channel_id or "ld-" in channel_id or "newtoken" in channel_id:
