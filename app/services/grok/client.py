        ...
        for outer_retry in range(MAX_OUTER_RETRY + 1):  # +1确保实际重试3次
            ...
            while retry_403_count <= max_403_retries:
                try:
                    ...
                    # 调试日志：显示使用的代理和 CF Clearance
                    cf = setting.grok_config.get("cf_clearance", "")
                    if proxy:
                        logger.info(f"[Client] 使用代理: {proxy[:50]}...")
                    else:
                        logger.warning("[Client] ⚠️ 未使用代理，可能导致 403 错误")
                    if cf:
                        logger.info(f"[Client] 使用 CF Clearance: {cf[:30]}...")
                    else:
                        logger.warning("[Client] ⚠️ 未配置 CF Clearance，可能导致 403 错误")
