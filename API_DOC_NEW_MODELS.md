...
|---|---|---|---|---|---|---|
| **`sdas-xh-minimax-h3-2k`** | **`2K` 极清** | **按次计费** | **3.00 元 / 次** | `15` 秒 (固定) | 图(9) / 视(3) / 音(3)<br>✅ 支持真人面部 | 海螺 H3 官方满血版，2K 极清，支持 9图3视3音，固定 15 秒生成。 |
| **`ld-sdas-cvk-pro-933-720p`** | **`720p` 高清** | **按次计费** | **4.00 元 / 次** | `4`–`15` 秒 | 图(9) / 视(3) / 音(3)<br>✅ 支持真人面部 | Seedance 2.0 720p 满血版（速大水渠道），画质清晰，支持多模态参考与真人人脸保持。 |
| **`sd2.0-480p`** | **`480p` 标准** | **按秒计费** | **0.35 元 / 秒** | `4`–`15` 秒 | 图(9) / 视(3) / 音(3)<br>✅ 支持真人面部 | Seedance 2.0 480p 满血版，概率卡脸，非常适合漫剧、线图及真人分身。 |
| **`sd2.5`** | **`720p` 高清** | **按次计费** | **5.00 元 / 次** | `30` 秒 (固定) | 图(50) (不支持视频/音频) | Seedance 2.5 全能多模态视频模型（hre3/chre3渠道），支持多达 50 张图片参考，不支持视频和音频参考，固定 30 秒生成。 |
| **`sd-c6`** | **`720p` 高清** | **按次计费** | **1.50 元 / 次** | `10` 秒 (固定) | 图(9) / 视(3) / 音(3) | Seedance 多模态视频模型，支持 9 张图片、3 段视频及 3 段音频参考，固定 10 秒生成。 |
...
#### 调用示例 1：调用海螺 `sdas-xh-minimax-h3-2k`（按次扣费 $3.00）
```bash
curl -X POST "https://grokai.zhubo.asia/v1/video/create" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "sdas-xh-minimax-h3-2k",
