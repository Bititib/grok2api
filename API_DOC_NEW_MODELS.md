...
|---|---|---|---|---|---|---|
| **`ld-sdas-h3-501-2k`** | **`2K` 极清** | **按次计费** | **2.50 元 / 次** | `5`–`15` 秒 | 图(5) / 视(0) / 音(1)<br>✅ 支持真人面部 | 海螺 H3 官方满血版，2K 画质极佳，画面表现逼真，生成速度快。 |
| **`sd2.0-480p`** | **`480p` 标准** | **按秒计费** | **0.30 元 / 秒** | `4`–`15` 秒 | 图(9) / 视(3) / 音(3)<br>✅ 支持真人面部 | Seedance 2.0 480p 满血版，概率卡脸，非常适合漫剧、线图及真人分身。 |
| **`sd2.0-fast-480p`** | **`480p` 标准** | **按秒计费** | **0.18 元 / 秒** | `4`–`15` 秒 | 图(9) / 视(3) / 音(3)<br>✅ 支持真人面部 | Seedance 2.0 480p 快速版，渲染速度极快，高性价比视频制作首选。 |
...
#### 调用示例 1：调用海螺 `ld-sdas-h3-501-2k`（按次扣费 $2.50）
```bash
...
#### 调用示例 2：调用 Seedance `sd2.0-480p`（按秒扣费，6秒共计 $1.80）
```bash
curl -X POST "https://grokai.zhubo.asia/v1/video/create" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
